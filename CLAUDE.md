...
### Core Components
- `cmd/mcp-broker-router/main.go`: Gateway binary entry point (broker + router)
- `cmd/main.go`: Controller binary entry point
...
### Dockerfiles
**IMPORTANT**: Use the correct Dockerfile for each component:
- `Dockerfile` - Builds **mcp-gateway** (broker + router) from `cmd/mcp-broker-router/main.go`
  - Binary: `/app/mcp_gateway`
  - Used for: Gateway deployments (broker + router pods)
- `Dockerfile.controller` - Builds **mcp-controller** from `cmd/main.go`
  - Binary: `/app/mcp_controller`
  - Used for: Controller deployments (watches CRDs, reconciles resources)
- Build scripts automatically use the correct Dockerfile
...
@/Users/memodi/workspaces/repos/mcp-gateway-console-plugin/Claude.md